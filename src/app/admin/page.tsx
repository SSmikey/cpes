      ...
      <div className="pt-6 px-4 md:px-12 max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pt-8 pb-4">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-none shadow-lg shadow-indigo-200 ring-4 ring-white/50">
        ...
        {stats ? (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-backwards delay-150">
            ...
            <section className="relative group">
              <div className="absolute inset-0 bg-gradient-to-r from-indigo-100/50 to-purple-100/50 rounded-none transform -rotate-1 scale-[1.01] blur-xl opacity-70 -z-10 group-hover:rotate-0 transition-transform duration-500" />

              <div className="bg-white/70 backdrop-blur-2xl rounded-none border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
                ...
                <div className="px-8 py-8 border-b border-indigo-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white/40">
                  <div className="flex items-center gap-4">
                    <div className="bg-indigo-50 p-3 rounded-none border border-indigo-100/50">
                  ...
                  <Button
                    ...
                    className="rounded-md bg-slate-900 hover:bg-slate-800 text-white shadow-lg shadow-slate-200 border-0 h-11 px-8 font-semibold tracking-wide transition-all hover:-translate-y-0.5"
                  >
                      ...
                      <tbody>
                        {stats.ranking.map((row) => (
                          <Fragment key={row.project_id}>
                            ...
                            `}
                            >
                              <td className="bg-white rounded-none py-5 px-8 border-y border-l border-indigo-50/50 shadow-sm group-hover:shadow-indigo-100/50 transition-all text-center">
                                <div className={`
                                size-12 rounded-none flex items-center justify-center font-black text-xl shadow-inner
                              ...
                              <td className="bg-white py-5 px-8 text-center border-y border-indigo-50/50 shadow-sm group-hover:shadow-indigo-100/50 transition-all">
                                <span className="text-sm font-medium text-slate-400 bg-slate-50 rounded-none px-2 py-1">{row.overall_sd.toFixed(2)}</span>
                              </td>
                              <td className="bg-white rounded-none py-5 px-4 text-center border-y border-r border-indigo-50/50 shadow-sm group-hover:shadow-indigo-100/50 transition-all">
                                <div className={`
                            ...
                            {expandedProject === row.project_id && (
                              <tr className="animate-in fade-in slide-in-from-top-1 duration-200">
                                <td colSpan={6} className="p-0">
                                  <div
                                    className="mx-4 mb-6 mt-[-12px] bg-slate-50/80 rounded-none border-x border-b border-indigo-100/50 p-8 shadow-inner overflow-hidden"
                                  >
                                    ...
                                    <div className="grid gap-3">
                                      {row.per_question && row.per_question.map((pq, idx) => (
                                        <div key={idx} className="flex items-center justify-between bg-white/80 p-4 rounded-none border border-indigo-50 shadow-sm hover:border-indigo-200 transition-colors">
            ...
            <section className="bg-white/70 backdrop-blur-2xl rounded-none border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
              <div className="px-12 py-8 border-b border-indigo-50/50 flex items-center justify-between bg-white/40">
                <div className="flex items-center gap-4">
                  <div className="bg-blue-50 p-3 rounded-none border border-blue-100/50">
                ...
                <Button variant="ghost" className="text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 rounded-md text-sm font-bold tracking-wide px-4">
                        ...
                        {stats.student_monitor.slice(0, 50).map((s) => {
                          ...
                          return (
                            <tr key={s.student_id} className="hover:bg-indigo-50/30 transition-colors group">
                              ...
                              <td className="px-6 py-5">
                                <Badge variant="secondary" className="bg-white border border-indigo-100 text-indigo-600 font-semibold shadow-sm rounded-none px-2.5 py-1 group-hover:bg-indigo-200 group-hover:text-indigo-800 transition-colors">
                                  {s.own_group}
                                </Badge>
                              </td>
                              ...
                              <td className="px-6 py-5">
                                <div className="h-3 w-full bg-slate-100 rounded-none overflow-hidden shadow-inner ring-1 ring-slate-50">
                                  <div
                                    className={`h-full rounded-none transition-all duration-1000 ease-out ${isComplete ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : 'bg-gradient-to-r from-blue-400 to-indigo-500'}`}
                              ...
                              <td className="px-12 py-5 text-right">
                                {isComplete ? (
                                  <div className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-md border border-emerald-100/50 shadow-sm">
                                    <CheckCircle2Icon className="size-3.5 fill-emerald-100" />
                                    <span className="text-[10px] font-bold tracking-wider">COMPLETED</span>
                                  </div>
                                ) : (
                                  <div className="inline-flex items-center gap-1.5 bg-slate-50 text-slate-500 px-3 py-1.5 rounded-md border border-slate-200/50">
          ...
          <div className="flex flex-col items-center justify-center py-32 opacity-30">
            <div className="animate-pulse flex flex-col items-center space-y-4">
              <div className="size-16 bg-indigo-200 rounded-none"></div>
              <div className="h-4 w-56 bg-indigo-100 rounded-none"></div>
              <div className="h-3 w-40 bg-indigo-50 rounded-none"></div>
            </div>
          </div>
        )}
      </div>
